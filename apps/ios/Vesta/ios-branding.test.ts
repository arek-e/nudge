...
describe("Vesta iOS branding", () => {
  test("bundle and Siri metadata expose Vesta without old spoken-name fallbacks", async () => {
    ...
    const project = await readFile(new URL("Vesta.xcodeproj/project.pbxproj", iosRoot), "utf8");
    const alternativeNames = readAlternativeNames(info);

    expect(info.CFBundleDisplayName).toBe("$(VESTA_DISPLAY_NAME)");
    expect(project).toContain("VESTA_DISPLAY_NAME = Vesta;");
  ...
  test("iOS build variants declare local, staging, and production environments", async () => {
    ...
    expect(project).toContain("PRODUCT_BUNDLE_IDENTIFIER = app.vesta.ios.local;");
    expect(project).toContain("PRODUCT_BUNDLE_IDENTIFIER = app.vesta.ios.staging;");
    expect(project).toContain("PRODUCT_BUNDLE_IDENTIFIER = app.vesta.ios;");
    expect(project).toContain('VESTA_ENGINE_URL = "http://localhost:8787";');
    expect(project).toContain(
      'VESTA_ENGINE_URL = "https://vesta-web-staging.teampitch.workers.dev";',
    );
    expect(project).toContain('VESTA_ENGINE_URL = "https://vesta-web.teampitch.workers.dev";');
    expect(buildSettingsForBundle(project, "app.vesta.ios.local")).toContain(
      'VESTA_CONVEX_DEPLOYMENT_URL = "https://grandiose-hamster-855.eu-west-1.convex.cloud";',
    );
    expect(buildSettingsForBundle(project, "app.vesta.ios.staging")).toContain(
      'VESTA_CONVEX_DEPLOYMENT_URL = "https://abundant-retriever-130.eu-west-1.convex.cloud";',
    );
    expect(buildSettingsForBundle(project, "app.vesta.ios")).toContain(
  ...
  test("staging build installs with the beta app icon", async () => {
    ...
    const normalPng = await expectPng(
      "Vesta/Assets.xcassets/AppIcon.appiconset/Icon-1024.png",
      1024,
      1024,
    );
    const stagingPng = await expectPng(
      "Vesta/Assets.xcassets/AppIconStaging.appiconset/Icon-1024.png",
      1024,
      1024,
    );
...
async function readAppIconSet(name: string) {
  const contents = await readFile(
    new URL(`Vesta/Assets.xcassets/${name}.appiconset/Contents.json`, iosRoot),
    "utf8",
  );
