...
struct VestaClientFailure: LocalizedError {
    let message: String
    ...
    init(error: Error) {
        if let failure = error as? VestaClientFailure {
            self = failure
            return
        }
...
extension VestaAPIClient: DependencyKey {
    static let liveValue = VestaAPIClient(
        getAgentRun: { runId in
            try await VestaAPI.getAgentRun(runId: runId)
        },
        getJournal: { localDate in
            try await VestaAPI.getJournal(localDate: localDate)
        },
        listActions: { limit in
            try await VestaAPI.listActions(limit: limit)
        },
        listCalendarDays: {
            try await VestaAPI.listCalendarDays()
        },
        listSignals: { limit in
            try await VestaAPI.listSignals(limit: limit)
        },
        saveDailyNote: { note, attachments, existingJournalText, trailingNote, localDate, treatsNoteAsFullBody in
            try await VestaAPI.saveDailyNote(
                note,
...
extension DependencyValues {
    var vestaAPIClient: VestaAPIClient {
        get { self[VestaAPIClient.self] }
        set { self[VestaAPIClient.self] = newValue }
    }
}

struct VestaNoteSyncClient {
...
private actor LiveVestaNoteSyncBox {
    ...
    func syncPending() async -> NoteSyncReceipt {
        do {
            ...
            return NoteSyncReceipt(
                failedCount: 1,
                lastErrorDescription: VestaClientFailure(error: error).localizedDescription
            )
        }
    }
    ...
    private func coordinatorOrThrow() async throws -> NoteSyncCoordinator {
        ...
        do {
            ...
            let client = await MainActor.run {
                ConvexNoteClient(client: vestaConvexClient)
            }
            let coordinator = NoteSyncCoordinator(store: store, client: client)
            self.coordinator = coordinator
            return coordinator
        } catch {
            throw VestaClientFailure(message: "Local sync unavailable")
        }
    }
}

extension VestaNoteSyncClient: DependencyKey {
    static let liveValue: VestaNoteSyncClient = {
        let box = LiveVestaNoteSyncBox()
        return VestaNoteSyncClient(
...
extension DependencyValues {
    var vestaNoteSyncClient: VestaNoteSyncClient {
        get { self[VestaNoteSyncClient.self] }
        set { self[VestaNoteSyncClient.self] = newValue }
    }
}

struct VestaNetworkStatusClient {
    var updates: @Sendable () -> AsyncStream<Bool>
}

extension VestaNetworkStatusClient: DependencyKey {
    static let liveValue = VestaNetworkStatusClient(
        updates: {
            AsyncStream { continuation in
                let monitor = NWPathMonitor()
                let queue = DispatchQueue(label: "app.vesta.network-monitor")
...
extension DependencyValues {
    var vestaNetworkStatusClient: VestaNetworkStatusClient {
        get { self[VestaNetworkStatusClient.self] }
        set { self[VestaNetworkStatusClient.self] = newValue }
    }
}

struct VestaStreakClient {
    var load: @Sendable () async -> VestaStreakSnapshot
    var save: @Sendable (_ dailyStreak: Int, _ lastStreakDate: String) async -> Void
}

struct VestaStreakSnapshot {
    let dailyStreak: Int
    let lastStreakDate: String
}

extension VestaStreakClient: DependencyKey {
    static let liveValue = VestaStreakClient(
        load: {
            let defaults = UserDefaults.standard
            return VestaStreakSnapshot(
                dailyStreak: defaults.integer(forKey: VestaCaptureFeature.dailyStreakKey),
                lastStreakDate: defaults.string(forKey: VestaCaptureFeature.lastStreakDateKey) ?? ""
            )
        },
        save: { dailyStreak, lastStreakDate in
            let defaults = UserDefaults.standard
            defaults.set(dailyStreak, forKey: VestaCaptureFeature.dailyStreakKey)
            defaults.set(lastStreakDate, forKey: VestaCaptureFeature.lastStreakDateKey)
        }
    )
}

extension DependencyValues {
    var vestaStreakClient: VestaStreakClient {
        get { self[VestaStreakClient.self] }
        set { self[VestaStreakClient.self] = newValue }
    }
}

struct VestaPendingCaptureResultContext {
    let noteText: String
    let saved: SavedCapture
}

struct VestaInitialContext {
    let localNote: LocalDailyNote?
    let streak: VestaStreakSnapshot
    let syncReceipt: NoteSyncReceipt
}

struct VestaRefreshContext {
...
struct VestaLocalDraftResponse {
    let localComposition: JournalSaveComposition
    let localNote: LocalDailyNote
    let request: VestaSubmissionRequest
}

struct VestaOptimisticSubmission {
    let localNote: LocalDailyNote
    let optimisticRowId: UUID
    let request: VestaSubmissionRequest
}

struct VestaRemoteSubmissionResponse {
    let optimisticRowId: UUID?
    let request: VestaSubmissionRequest
    let saved: SavedCapture
}

struct VestaSubmissionFailure: Error {
    let error: VestaClientFailure
    let savedOnDevice: Bool
}

@Reducer
struct VestaCaptureFeature {
    static let dailyStreakKey = "vesta.dailyStreak"
    static let lastStreakDateKey = "vesta.lastStreakDate"
    ...
    struct State {
        ...
        var navigationPath: [VestaDestination] = []
        ...
        var pendingResultContext: VestaPendingCaptureResultContext?
        ...
        mutating func updateDailyStreak(for localDate: String) -> Bool {
            ...
            if VestaCaptureFeature.previousLocalDate(from: localDate) == lastStreakDate {
                dailyStreak += 1
            } else {
                dailyStreak = 1
            }
    ...
    enum Action: BindableAction {
        ...
        case activeSheetChanged(VestaSheet?)
        case autosaveDraftResponse(Result<LocalDailyNote, VestaClientFailure>)
        case binding(BindingAction<State>)
        case cameraButtonTapped(isAvailable: Bool)
        case cancelProcessingEditInterruption
        case chromeActionTapped(VestaChromeAction)
        ...
        case initialContextLoaded(VestaInitialContext)
        case localDraftSaved(VestaLocalDraftResponse)
        case networkStatusChanged(Bool)
        case openAttachment(CaptureAttachment)
        case optimisticLocalSubmissionCompleted(VestaOptimisticSubmission)
        ...
        case refreshContext
        case refreshContextResponse(Result<VestaRefreshContext, VestaClientFailure>)
        case remoteSubmissionResponse(Result<VestaRemoteSubmissionResponse, VestaSubmissionFailure>)
        case removeAttachment(CaptureAttachment)
        case settingsSignOutDismissed
        case settingsSignOutTapped
        case settingsSignOutResponse(Result<Void, VestaClientFailure>)
        case submit
        case submitResponse(Result<VestaRemoteSubmissionResponse, VestaSubmissionFailure>)
    ...
    @Dependency(\.vestaAPIClient) private var api
    @Dependency(\.vestaAuthClient) private var auth
    @Dependency(\.vestaNetworkStatusClient) private var network
    @Dependency(\.vestaNoteSyncClient) private var noteSync
    @Dependency(\.vestaStreakClient) private var streak
        ...
        Reduce { state, action in
                ...
                return .run { send in
                    do {
                        ...
                        if let localNote = try? await noteSync.localDailyNote(localDate) {
                            ...
                            await send(.refreshContextResponse(.failure(VestaClientFailure(error: error))))
                        }
                    }
                }
            ...
            case .refreshContextResponse(.success(let context)):
                ...
                if isProcessing && tracksPendingResult {
                    ...
                    state.pendingResultContext = nil
                    state.stage = .saved
                    state.statusMessage = "Saved to Vesta"
            ...
            case .settingsSignOutTapped:
                ...
                return .run { send in
                    do {
                        ...
                        await send(.settingsSignOutResponse(.failure(VestaClientFailure(error: error))))
                    }
                }
            ...
            case .submitResponse(.success(let response)):
                ...
                if completion.showsResultRow {
                    ...
                    state.latestResult = nil
                    state.pendingResultContext = VestaPendingCaptureResultContext(noteText: request.submission.noteText, saved: response.saved)
                }
    ...
    private func scheduleDraftAutosave(state: inout State) -> Effect<Action> {
        ...
        return .run { send in
            do {
                ...
                await send(.autosaveDraftResponse(.failure(VestaClientFailure(error: error))))
            }
        }
                ...
                if let localNote = localNoteForOptimisticSubmission, mediaAttachments.isEmpty {
                    ...
                    do {
                        ...
                        await send(.remoteSubmissionResponse(.failure(
                            VestaSubmissionFailure(
                                error: VestaClientFailure(error: error),
                                savedOnDevice: true
                            )
                        )))
                    }
