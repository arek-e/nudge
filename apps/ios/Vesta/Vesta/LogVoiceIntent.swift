...
struct LogVoiceIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        do {
            let response = try await VestaAPI.logVoice(spokenText: text)
            if response.route == "reasoning_candidate" {
                await VestaNotifications.sendVoiceLogProcessed()
            }
            return .result(dialog: IntentDialog(stringLiteral: response.spokenResponse))
        } catch {
            return .result(
                dialog: IntentDialog(
                    stringLiteral: "I heard it, but couldn't reach the Vesta Engine at \(VestaAPI.configuredEngineURL)."
                )
            )
        }
    }
...
enum VestaNotifications {
    ...
    static func sendVoiceLogProcessed() async {
        ...
        content.title = "Vesta"
        content.body = "Your Siri log is ready in Vesta."
        content.sound = .default

        let request = UNNotificationRequest(
            identifier: "vesta.voice.\(UUID().uuidString)",
            content: content,
