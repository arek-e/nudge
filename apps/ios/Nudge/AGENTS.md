# Nudge iOS Agent Instructions

Scope: this file applies to work under `apps/ios/Nudge`. Keep it short, current, and operational. Put long design history in `docs/` and keep this file focused on the commands, boundaries, and architecture rules an agent must follow while editing the iOS app.

## Architecture

The iOS app is a SwiftUI App Surface backed by The Composable Architecture.

- `NudgeRootFeature` in `Nudge/NudgeAuth.swift` owns authenticated root state.
- `NudgeCaptureFeature` in `Nudge/NudgeCaptureFeature.swift` owns capture, navigation, sheet/modal state, refresh, submission, local note sync, network reachability, streak persistence, and settings sign-out effects.
- SwiftUI views receive `StoreOf<NudgeCaptureFeature>` or scoped stores and send actions. They should not create app-level `@StateObject` view models for product state.
- `NudgeCaptureViewModel.swift` is intentionally no longer a view model. It only hosts capture attachment/result value types that are still used by the reducer and views.
- Keep pure policies in `NudgePresentation.swift`, `NudgeAPI.swift`, and `CalendarStats.swift` as small deterministic domain seams. Reducers should call those policies instead of duplicating their rules.
...
- Keep Engine HTTP calls behind `NudgeAPIClient`.
- Keep Convex/local-note behavior behind `NudgeNoteSyncClient`.
- Keep auth behavior behind `NudgeAuthClient`.
- Keep network reachability behind `NudgeNetworkStatusClient`.
- Keep streak persistence behind `NudgeStreakClient`.
...
```sh
bun test apps/ios/Nudge/ios-branding.test.ts
```

For architecture migrations, also check that broad legacy view-model state has not returned:

```sh
rg "NudgeCaptureViewModel|@StateObject private var model|@ObservedObject var model" apps/ios/Nudge/Nudge
```

## Migration Guardrails

- Do not move Nudge Engine rules into the iOS app. The app is an App Surface; durable product behavior belongs behind the Engine.
