...
struct LogVoiceIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        do {
            let response = try await NudgeAPI.logVoice(spokenText: text)
            if response.route == "reasoning_candidate" {
                await NudgeNotifications.sendVoiceLogProcessed()
            }
            return .result(dialog: IntentDialog(stringLiteral: response.spokenResponse))
        } catch {
            return .result(
                dialog: IntentDialog(
                    stringLiteral: "I heard it, but couldn't reach the Nudge Engine at \(NudgeAPI.configuredEngineURL)."
                )
            )
        }
    }
...
enum NudgeNotifications {
    ...
    static func sendVoiceLogProcessed() async {
        ...
        content.title = "Nudge"
        content.body = "Your Siri log is ready in Nudge."
        content.sound = .default

        let request = UNNotificationRequest(
            identifier: "nudge.voice.\(UUID().uuidString)",
            content: content,
