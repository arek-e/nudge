...
SwiftUI remains the iOS choice because mobile capture, Siri/App Intents, calendar permissions, notifications, and native review affordances matter more there than sharing a React editor. Raycast remains a TypeScript command surface for low-friction capture, search, menu-bar status, and lightweight review, but it should call the Engine API or auth-scoped Convex functions instead of owning agent logic or using runtime Convex secrets.
