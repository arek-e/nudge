...
- Domain-specific sub-agents own isolated responsibilities, starting with `LoopIntakeAgent` for message intake and reviewable draft creation.
...
- Think is the intended next harness once we introduce real model turns, streaming chat, session memory, skills, and model-selected tools.
...
- Think adoption should happen as a vertical slice around one model-backed turn, not as a wholesale rewrite.
