...
Internal Worker eval runs are available at `POST /__internal/evals/agent` with `x-nudge-eval-secret` set to the internal agent secret. The route passes the Worker D1 and R2 bindings into `createEvalTraceSink`.
