...
- `apps/web/src/Layers/NudgeAppLive.ts` owns the live Worker layer and runtime cache helpers. It resolves environment bindings, the dev user, durable namespaces, model config, optional Turbopuffer config, and the concrete `Db` layer.
- `apps/web/src/db-layer.ts` owns deployed product-storage adapter selection. It provides the Convex-backed `Db` adapter for the Nudge Engine and fails fast when `CONVEX_URL` or `CONVEX_RUNTIME_SECRET` is missing.
- `packages/db` owns the persistence interface and memory adapter. Runtime code should call the `Db` Effect service, not storage clients directly.
- `packages/db-convex` owns the Convex runtime adapter behind the `Db` seam.
