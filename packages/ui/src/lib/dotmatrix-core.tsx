...
// oxlint-disable-next-line import/no-unassigned-import -- Dot Matrix registry CSS side effect.
...
export function DotMatrixBase({
  ...
  const boxOptions: { boxSize?: number; minSize?: number } = {};
  if (boxSize !== undefined) boxOptions.boxSize = boxSize;
  if (minSize !== undefined) boxOptions.minSize = minSize;
  const { outerDim, useWrapper } = resolveDmxBoxOuterDim(boxOptions);
