...
describe("observability", () => {
  ...
  test("maps wide events to trace_events rows without knowing D1", () => {
    const row = buildTraceEventRow({
      event: {
        ...
        durationMs: 8,
        sampleReason: "default",
        errorMessage: "secret token value",
        errorStack: "Error: secret token value\n    at test",
        timestamp: "2026-06-12T10:00:00.000Z",
      },
      id: "event-1",
    ...
    expect(String(row.values[16])).toContain("[redacted]");
    expect(String(row.values[16])).not.toContain("secret token value");
    expect(String(row.values[16])).not.toContain("errorStack");
  });
  ...
  test("redacts exception messages and stack traces in span attributes", () => {
    const error = new Error("boom");
    expect(safeExceptionAttributes(error)).toMatchObject({
      "exception.type": "Error",
      "exception.message": "[redacted]",
    });
    expect(safeExceptionAttributes(error)).not.toHaveProperty("exception.stacktrace");
  });
